...
# Set API key as environment variable for ADK compatibility
...
async def generate_podcast_audio(podcast_script: str, tool_context: ToolContext, filename: Optional[str] = None) -> Dict[str, str]:
    if filename is None:
        filename = config.PODCAST_FILENAME.replace('.wav', '')
                            ...
                            types.SpeakerVoiceConfig(
                                ...
                                voice_config=types.VoiceConfig(
                                    prebuilt_voice_config=types.PrebuiltVoiceConfig(
                                        voice_name=config.SPEAKER_CONFIGS["Joe"]["voice_name"]
                                    )
                                )
                            ),
                            types.SpeakerVoiceConfig(
                                ...
                                voice_config=types.VoiceConfig(
                                    prebuilt_voice_config=types.PrebuiltVoiceConfig(
                                        voice_name=config.SPEAKER_CONFIGS["Jane"]["voice_name"]
                                    )
                                )
                            )
                        ]
...
def filter_news_sources_callback(tool, args, tool_context):
    if tool.name == "google_search":
        ...
        if any(f"site:{domain}" in original_query.lower() for domain in config.WHITELIST_DOMAINS):
            return None
        whitelist_query_part = " OR ".join([f"site:{domain}" for domain in config.WHITELIST_DOMAINS])
